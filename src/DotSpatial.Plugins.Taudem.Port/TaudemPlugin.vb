...
Public Class TaudemPlugin
    ...
    '<Import(GetType(Heiflow.Applications.VHFAppManager))>
    Public Property Manager As VHFAppManager
...
#Region "Start and Stop Functions"
    Public Overrides Sub Activate()
        Dim wd_item As SimpleActionItem
        ...
        wd_item = New SimpleActionItem("kModel", "Watershed Delineation", AddressOf AutoWatershedDelineation_Click)
        wd_item.LargeImage = My.Resources.watershed_dl
        wd_item.Key = "kWatershedDelineation"
        wd_item.GroupCaption = "Delineation"
        wd_item.ToolTipText = "Watershed Delineation"
        wd_item.SortOrder = 0

        App.HeaderControl.Add(wd_item)
        'Dim nil As Object
        'nil = Nothing
        'With App.Map.Menus
...
#End Region
    ...
    Private Sub AutoWatershedDelineation_Click(ByVal sender As Object, ByVal e As EventArgs)
        Dim vhf As VHFAppManager
        Dim prj As IProject
        If g_AutoForm Is Nothing Or g_AutoForm.IsDisposed Then
            g_AutoForm = New frmAutomatic_v3
        End If
        g_AutoForm.Icon = My.Resources.vhf
        g_AutoForm.WindowState = Windows.Forms.FormWindowState.Normal
        vhf = VHFAppManager.Instance
        prj = vhf.ProjectController.ProjectService.Project
        g_AutoForm.Initialize(Me, prj)
        g_AutoForm.Show()
