...
{
    public class CompareTimeRangeGWHead : MapLayerRequiredTool
    {
        public CompareTimeRangeGWHead()
        {
            Name = "Compare Groundwater Head During A Period";
            Category = "Postprocessing";
            Description = "Compare Groundwater Head During A Period";
            Version = "1.0.0.0";
        ...
        [Category("Parameter")]
        [Description("The start time step")]
            ...
            if (mf != null)
            {
                ...
                if (buf.Any())
                {
                    ...
                    if (fhd.DataCube.IsAllocated(layer))
                    {
                        ...
                        if (EndTimeStep <= 0 || EndTimeStep > fhd.DataCube.Size[1])
                            EndTimeStep = fhd.DataCube.Size[1];

                        if (StartTimeStep <= 0 || StartTimeStep > fhd.DataCube.Size[1])
                            StartTimeStep = 1;

                        if (StartTimeStep > EndTimeStep)
                            StartTimeStep = EndTimeStep - 1;
                        int nstep = EndTimeStep - StartTimeStep + 1;
                        if(nstep <= 0 )
                        {
                            cancelProgressHandler.Progress("Package_Tool", 100, "Invalid time steps");
                            return false;
                        }
