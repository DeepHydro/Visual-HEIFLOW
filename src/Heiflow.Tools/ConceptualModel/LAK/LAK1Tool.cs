...
{
    public  class LAK1Tool : MapLayerRequiredTool
    {
        private IMapLayerDescriptor _LakeFeatureLayerDescriptor;
        public LAK1Tool()
        {
            Name = "Create LAK Package1";
            Category = Cat_CMG;
            SubCategory = "LAK";
        ...
        public override bool Execute(ICancelProgressHandler cancelProgressHandler)
        {
            ...
            if (mf != null)
            {
                ...
                for (int i = 0; i < grid.RowCount; i++)
                {
                    for (int j = 0; j < grid.ColumnCount; j++)
                    {
                        if (grid.MFIBound[0, i, j] == 0 && grid.MFIBound[1, i, j] != 0)
                        {
                            for (int k = 1; k < grid.ActualLayerCount; k++)
                            {
                                grid.MFIBound[k, i, j] = 0;
                            }
                        }
                    }
                }
        ...
        public override void AfterExecution(object args)
        {
            ...
            if (model != null)
            {
                ...
                for (int i = 0; i < hru_type.ValueCount; i++)
                {
                    lake_hru_id.SetValue(0, i, 0, 0);
                    if (int.Parse(hru_type.GetValue(0, i, 0).ToString()) == 2)
                    {
                        hru_type.SetValue(0, i, 0, 0);
                        cov_type.SetValue(0, i, 0, 0);
                    } 
                }
